...
struct s5pc110_audio_device {
    ...
    int in_call;
    float voice_volume;
    struct s5pc110_stream_in *active_input;
    struct s5pc110_stream_out *active_output;
    bool mic_mute;
    int tty_mode;
...
struct s5pc110_stream_out {
    ...
    int standby;
    struct echo_reference_itfe *echo_reference;
    struct s5pc110_audio_device *dev;
    int write_threshold;
    bool low_power;
};
...
static void select_output_device(struct s5pc110_audio_device *adev);
static void select_input_device(struct s5pc110_audio_device *adev);
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume);
static int do_input_standby(struct s5pc110_stream_in *in);
static int do_output_standby(struct s5pc110_stream_out *out);
...
static int start_call(struct s5pc110_audio_device *adev)
{
...
static void end_call(struct s5pc110_audio_device *adev)
{
...
static void set_eq_filter(struct s5pc110_audio_device *adev)
{
...
{
    struct s5pc110_audio_device *adev = (struct s5pc110_audio_device *)data;
...
static void set_incall_device(struct s5pc110_audio_device *adev)
{
    int device_type;
...
static void set_input_volumes(struct s5pc110_audio_device *adev, int main_mic_on,
...
static void set_output_volumes(struct s5pc110_audio_device *adev, bool tty_volume)
{
    unsigned int channel;
    int speaker_volume;
    int headset_volume;
...
static void force_all_standby(struct s5pc110_audio_device *adev)
{
    struct s5pc110_stream_in *in;
    struct s5pc110_stream_out *out;
...
static void select_mode(struct s5pc110_audio_device *adev)
{
    if (adev->mode == AUDIO_MODE_IN_CALL) {
...
static int start_output_stream(struct s5pc110_stream_out *out)
{
    struct s5pc110_audio_device *adev = out->dev;
...
static void add_echo_reference(struct s5pc110_stream_out *out,
...
static void remove_echo_reference(struct s5pc110_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct s5pc110_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct s5pc110_audio_device *adev,
...
static int get_playback_delay(struct s5pc110_stream_out *out,
                       size_t frames,
...
{
    struct s5pc110_stream_out *out = (struct s5pc110_stream_out *)stream;
...
static int start_input_stream(struct s5pc110_stream_in *in)
{
    int ret = 0;
    struct s5pc110_audio_device *adev = in->dev;
...
{
    struct s5pc110_stream_in *in = (struct s5pc110_stream_in *)stream;
...
static void get_capture_delay(struct s5pc110_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct s5pc110_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct s5pc110_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct s5pc110_stream_in *)((char *)buffer_provider -
                                   offsetof(struct s5pc110_stream_in, buf_provider));
...
static ssize_t read_frames(struct s5pc110_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct s5pc110_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
{
    struct s5pc110_audio_device *ladev = (struct s5pc110_audio_device *)dev;
    struct s5pc110_stream_out *out;
    int ret;

    out = (struct s5pc110_stream_out *)calloc(1, sizeof(struct s5pc110_stream_out));
    if (!out)
        return -ENOMEM;
...
{
    struct s5pc110_audio_device *adev = (struct s5pc110_audio_device *)dev;
    struct str_parms *parms;
    char *str;
    char value[32];
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    in = (struct s5pc110_stream_in *)calloc(1, sizeof(struct s5pc110_stream_in));
    if (!in)
        return -ENOMEM;
...
{
    struct s5pc110_audio_device *adev = (struct s5pc110_audio_device *)device;
...
static int adev_open(const hw_module_t* module, const char* name,
                     hw_device_t** device)
{
    struct s5pc110_audio_device *adev;
    int ret;

    if (strcmp(name, AUDIO_HARDWARE_INTERFACE) != 0)
        return -EINVAL;

    adev = calloc(1, sizeof(struct s5pc110_audio_device));
    if (!adev)
        return -ENOMEM;
